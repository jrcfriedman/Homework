...
rm(df)
# Adding calculation of episode magnitudes  * this worked :-)
backslide <- function(df, x, group_var, neg.threshold, pos.threshold, stagnation) {
  group_var <- enquo(group_var)
  print(group_var)
...
backslide(df = vdem, x = v2x_polyarchy, group_var = country_id, new.column = diff)
...
# This worked
sum.backslide <- function(df, x, group_var) {
  group_var <- enquo(group_var)
  print(group_var)
...
sum.backslide(vdem, v2x_polyarchy, country_id)
